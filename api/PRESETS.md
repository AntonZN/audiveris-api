...
> **Как пресеты применяются сейчас (2026-09-10).** Распознаёт homr (пайплайн
> `omr/`), Audiveris из общей цепочки выведен, поэтому большинство пресетов
> принимается, но на результат не влияет. Исключение — ударные: `drums` и
> `drums_1line` уводят задачу прямо в Audiveris, минуя homr (он ударных не знает).
> Для `drums_1line` интервал стана оценивается по высоте головок и подставляется
> сам; при провале Audiveris пробуются соседние значения. Провал Audiveris на
> ударных — ошибка задачи, отката на homr нет. Код — `api/drums.py`.

...
сыром файле. Audiveris из неё выведен — он остался в проекте для другой задачи,
но ни один запрос на распознавание его не запускает. Здесь сторожится и то, и
другое: провал omr не проваливает задачу, пока есть второе звено, — и Audiveris
не зовётся даже когда звенья кончились.
"""
